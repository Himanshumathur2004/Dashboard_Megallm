...
import { fetchAllArticles } from "./fetch-articles.js";
import { draftComments } from "./draft-comments.js";
import { DATA_DIR, POSTS_FILE, DRAFTS_FILE, SIMS_DIR } from "./paths.js";
...
app.post("/api/fetch", async (_req, res) => {
  try {
    const p = await fetchAllArticles();
...
app.get("/api/simulations", async (_req, res) => {
  ...
  res.json(sims.map((s, i) => ({
    ...
    primaryTag: s.metadata?.primaryTag || s.metadata?.subreddit || s.post?.primaryTag || s.post?.subreddit || "?",
...
<title>Dev.to Comment Assistant</title>
<style>
...
.fetch-row{padding:12px 16px;border-bottom:1px solid #1a1a1a;display:flex;justify-content:space-between;align-items:flex-start;gap:12px}
.fetch-row:hover{background:#111}
.fetch-row a{color:#60a5fa;text-decoration:none;font-weight:600;flex:1;font-size:14px}
.fetch-row a:hover{text-decoration:underline}
...
.fetched-panel{background:#141414;border:1px solid #262626;border-radius:8px;overflow:hidden}
.fetched-panel-h{padding:10px 14px;background:#0d0d0d;border-bottom:1px solid #262626;font-size:13px;font-weight:600;color:#e0e0e0;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap}
...
<body>
<div class="c">
<h1>Dev.to Comment Assistant</h1>
<p class="sub">4 options per article &middot; scored for megallm benefit &middot; viral detection</p>

<div class="actions">
  <button onclick="fetchPosts()" id="fetchBtn">Fetch New Articles</button>
  <button onclick="draftAll()" id="draftBtn">Draft Comments (4 per article)</button>
...
<div class="tabs">
  ...
  <div class="tab" data-tab="fetched" onclick="switchTab('fetched')">Fetched articles</div>
...
<script>
let tab='draft',all=[],sims=[],currentSim=null,cachedArticles=[];
...
async function load(){
  ...
  try{
    ...
    if(!ar.ok)throw new Error('articles HTTP '+ar.status);
    all=await dr.json();
    ...
    console.error(e);
    if(errEl)errEl.textContent='Load error: '+e.message+' — check server terminal.';
  }
}
...
function renderFetchedPanel(){
  ...
  if(!cachedArticles.length){
    el.innerHTML='<p class="hint" style="margin:0 0 8px 0">No articles in cache — click <strong>Fetch New Articles</strong> to load dev.to posts here and in the <strong>Fetched articles</strong> tab.</p>';
    return;
  }
  const rows=cachedArticles.map(function(a){
    return '<div class="fetch-row">'+
      ...
      '<span class="badge b-ups">'+(a.ups||0)+' ❤</span>'+
      '<span class="badge b-low">'+(a.numComments||0)+' cmts</span>'+
      '</div></div>';
  }).join('');
  el.innerHTML=
    '<div class="fetched-panel">'+
    '<div class="fetched-panel-h"><span>Fetched articles</span><span class="l">from data/posts.json · scroll for full list</span><span class="badge b-ups">'+cachedArticles.length+'</span></div>'+
    '<div class="fetched-panel-body">'+rows+'</div>'+
    '</div>';
}
...
function renderStats(){
  ...
  document.getElementById('stats').innerHTML=
    st('Articles cached',cachedArticles.length)+st('Drafts',s.draft)+st('Posted',s.posted)+st('High Priority',s.highPri)+st('Avg Score',s.avgScore);
}
...
function render(){
  ...
  if(!filtered.length){
    ...
    if(tab==='draft'&&cachedArticles.length){
      msg+='<p class="hint">You have <strong>'+cachedArticles.length+'</strong> articles in cache. Click <strong>Draft Comments</strong> to generate copy-ready options, or open the <strong>Fetched articles</strong> tab to browse them.</p>';
    }
...
function renderFetched(){
  if(!cachedArticles.length){
    document.getElementById('content').innerHTML='<div class="empty">no cached articles yet. click <strong>Fetch New Articles</strong> first.</div>';
    return;
  }
  const rows=cachedArticles.map(function(a){
    return '<div class="fetch-row">'+
  ...
  document.getElementById('content').innerHTML=
    '<div class="post-group" style="margin-bottom:12px"><div class="post-header"><span style="font-size:13px;color:#888">'+cachedArticles.length+' articles in data/posts.json — ready for drafting</span></div></div>'+
    rows;
}
...
function renderGroup(g){
  ...
  return '<div class="post-group">'+
    '<div class="post-header">'+
      ...
      '<div class="badges">'+
        '<span class="badge b-sub">#'+esc(p.primaryTag || p.subreddit)+'</span>'+
        '<span class="badge b-ups">'+p.postUps+' ❤</span>'+
  ...
  sims.forEach(function(s){
    ...
    h+='<li><div class="sim-card" data-file="'+esc(s.filename)+'">'+
      '<div class="sim-card-top">'+
        ...
        '<div class="badges">'+
          '<span class="badge b-sub">#'+esc(s.primaryTag || s.subreddit)+'</span>'+
...
function renderSimDetail(){
  ...
  h+='<span class="badge b-sub">#'+esc(p.primaryTag || p.subreddit)+'</span>';
  if(p.flair)h+='<span class="badge b-low">'+esc(p.flair)+'</span>';
  h+='<span>u/'+esc(p.author)+'</span>';
  h+='<span>'+p.upvotes+' ❤</span>';
...
async function posted(i){await fetch('/api/drafts/'+i+'/posted',{method:'POST'});all[i].status='posted';renderStats();renderFetchedPanel();render()}
async function skip(i){await fetch('/api/drafts/'+i+'/skip',{method:'POST'});all[i].status='skipped';renderStats();renderFetchedPanel();render()}
...
async function fetchPosts(){
  ...
  try{
    ...
    await load();
    setTimeout(function(){b.textContent='Fetch New Articles';b.disabled=false},2000);
  }catch(e){
    b.textContent='Error';
    b.disabled=false;
    setTimeout(function(){b.textContent='Fetch New Articles'},3000);
  }
}

async function draftAll(){
  const b=document.getElementById('draftBtn');b.disabled=true;b.textContent='Drafting (takes ~1 min)...';
  try{const r=await(await fetch('/api/draft',{method:'POST'})).json();b.textContent='Drafted '+r.count+'!';await load();setTimeout(()=>{b.textContent='Draft Comments (4 per article)';b.disabled=false},2000)}
...
app.listen(CONFIG.port, () => {
  console.log("");
  console.log("  Dev.to Comment Assistant v2");
  console.log("  http://localhost:" + CONFIG.port);
  console.log("");
  console.log("  Features:");
  console.log("  - 4 comment options per article (incl. witty)");
  console.log("  - MegaLLM benefit score (0-100)");
  console.log("  - Viral potential detection (HIGH/MEDIUM/LOW)");
  console.log("  - Grouped by article, sorted by score");
  console.log("  - Simulated threads: full comment tree with 30-40 users");
  console.log("");
});

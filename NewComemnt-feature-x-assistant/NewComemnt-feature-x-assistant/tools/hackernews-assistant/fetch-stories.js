...
/**
 * Check if a story title matches any of the topic keywords.
 * Used to filter for AI/LLM relevant stories only.
 */
function matchesTopic(title) {
  const t = (title || "").toLowerCase();
  const { topicKeywords } = CONFIG.hn;
  
  // Match if any keyword appears in title
  for (const keyword of topicKeywords) {
    const wordBoundary = new RegExp(`\\b${keyword}\\b`, "i");
    if (wordBoundary.test(t)) {
...
export async function fetchAllStories() {
  ...
  const { list, idFetchLimit, minScore, maxComments, maxStories, topLevelCommentSample, strictTopicFilter } =
    CONFIG.hn;
  const listName = ["top", "new", "best"].includes(list) ? list : "top";

  console.log(`\nFetching HN /${listName}stories (up to ${idFetchLimit} ids)...\n`);
  console.log(`📌 Topic Filter: ${strictTopicFilter ? "STRICT (AI/LLM only)" : "RELAXED (all + AI/LLM boost)"}`);
  console.log(`🎯 Keywords: ${CONFIG.hn.topicKeywords.length} topics being monitored\n`);
  ...
  for (const id of slice) {
    ...
    try {
      ...
      // Topic filtering
      const isTopicRelevant = matchesTopic(item.title);
      if (strictTopicFilter && !isTopicRelevant) {
        // Skip non-relevant stories in strict mode
        continue;
      }
      ...
      post.isTopicRelevant = isTopicRelevant; // Store for reference
      posts.push(post);
      
      // Log with topic indicator
      const topicBadge = isTopicRelevant ? "🎯" : "📄";
      console.log(`  ${topicBadge} ${post.primaryTag} ${score}pts ${descendants}cmt: "${post.title.slice(0, 52)}..."`);
    } catch (err) {

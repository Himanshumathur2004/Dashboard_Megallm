...
def generate_blogs():
    ...
    # For Quora/Medium/Dev.to/Tumblr/Blogger/AGIorBust accounts: scrape fresh articles first, use them as content source
    articles_pool = []
    agiorbust_account_id = getattr(Config, "AGIORBUST_ACCOUNT_ID", "account_3")
    if account_id in {Config.QUORA_ACCOUNT_ID, Config.MEDIUM_ACCOUNT_ID, devto_account_id, tumblr_account_id, blogger_account_id, agiorbust_account_id} and not db.is_memory:
        try:
    ...
    # Check if this is an AGIorBust generation
    is_agiorbust = account_id == Config.AGIORBUST_ACCOUNT_ID
    if is_agiorbust:
        topics_to_generate = {tid: Config.AGIORBUST_TOPICS[tid]["blogs_per_cycle"] for tid in Config.AGIORBUST_TOPICS}
        logger.info(f"[AGIORBUST_GEN] Using {len(topics_to_generate)} AGIorBust topics")
    else:
        logger.info(f"[GEN] Using {len(topics_to_generate)} MegaLLM topics")

    try:
        for topic_id, count in topics_to_generate.items():
            if is_agiorbust:
                if topic_id not in Config.AGIORBUST_TOPICS:
                    continue
                topic_info = Config.AGIORBUST_TOPICS[topic_id]
            else:
            ...
            for i in range(count):
                ...
                if article:
                    ...
                    if is_agiorbust:
                        blog_data = blog_generator.generate_agiorbust_blog(

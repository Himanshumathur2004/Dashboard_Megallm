...
class BlogGenerator:
    ...
    def __init__(
        ...
        self._fallback_targets = self._build_fallback_targets()
        logger.info(f"[INIT] BlogGenerator initialized with model: {self.model}, base_url: {self.base_url}, max_retries: {max_retries}")
        logger.info(f"[INIT] Loaded {len(self._fallback_targets)} fallback target(s)")

    def _build_fallback_targets(self) -> List[Dict[str, str]]:
        """Build ordered provider/model targets used for chat completion fallback."""
        targets: List[Dict[str, str]] = []
        seen = set()

        if self.fallback_providers:
            for provider in self.fallback_providers:
                provider_name = (provider.get("name") or "provider").strip() or "provider"
                ...
                for model in models:
                    ...
                    if not model_name:
                        continue
                    dedupe_key = (provider_name.lower(), base_url.lower(), model_name)
        ...
        if not targets and self.base_url and self.api_key and self.model:
            targets.append(
                {
                    "provider": "primary",
    ...
    def _make_chat_completion_with_fallback(self, payload: Dict[str, Any], timeout: int = 60) -> Optional[requests.Response]:
        """Try chat completions across configured providers/models until one succeeds."""
        total = len(self._fallback_targets)
        if total == 0:
            logger.error("[FALLBACK] No fallback targets available for chat completions")
            return None

        payload = self._inject_no_em_dash_directive(payload)

        for index, target in enumerate(self._fallback_targets, start=1):
            url = f"{target['base_url']}/chat/completions"
            request_payload = copy.deepcopy(payload)
            request_payload["model"] = target["model"]
            
            # Don't inject response_format - MegaLLM doesn't honor it and it causes issues
            # Instead, rely on explicit JSON instructions in the prompt itself
            ...
            logger.info(
                f"[FALLBACK] Attempt {index}/{total}: provider={target['provider']} model={target['model']}"
            )
            ...
            if response is not None:
                if index > 1:
                    logger.info(
                        f"[FALLBACK] Recovered on provider={target['provider']} model={target['model']}"
                    )
                return response

            logger.warning(
                f"[FALLBACK] Failed provider={target['provider']} model={target['model']}, trying next"
            )

        logger.error("[FALLBACK] Exhausted all provider/model fallback targets")
        return None

    def _inject_no_em_dash_directive(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Ensure every model call includes a global no-em-dash writing rule."""
        directive = "Hard rule: do not use em-dashes (—) anywhere in titles or body text. Use commas, colons, or periods instead."
        request_payload = copy.deepcopy(payload or {})
        ...
        for message in messages:
            ...
            if not isinstance(content, str):
                continue
            if "em-dash" in content.lower() or "em dash" in content.lower():
                return request_payload
    ...
    def _remove_em_dashes(self, text: str) -> str:
        """Replace em-dashes with normal hyphens to enforce editorial policy."""
        if not text:
            return text
        return text.replace("—", "-")

    def _extract_message_content(self, response: requests.Response) -> Optional[str]:
        """Extract message content from API response, handling both 'content' and 'reasoning' fields."""
        try:
            data = response.json()
            ...
            content = message.get("content")
            if content:
                logger.debug(f"[EXTRACT] Using 'content' field ({len(content)} chars): {content[:100]}")
                return content
            
            # Fallback to 'reasoning' (some MegaLLM providers return response here)
            reasoning = message.get("reasoning")
            if reasoning:
                logger.debug(f"[EXTRACT] Using 'reasoning' field ({len(reasoning)} chars): {reasoning[:100]}")
                return reasoning
            
            logger.warning("[EXTRACT] Neither 'content' nor 'reasoning' found in response")
            logger.warning(f"[EXTRACT] Full message keys: {message.keys()}")
            return None
        except Exception as e:
            logger.error(f"[EXTRACT] Failed to extract message content: {e}")
            return None
    ...
    def _sanitize_professional_title(self, title: str, topic: str = "") -> str:
        ...
        raw = raw.strip("\"'")
        raw = self._remove_em_dashes(raw)
...
{{
  ...
  "body": "Rewritten body in markdown"
    ...
    def _generate_devto_article_from_source(self, article_title: str, article_content: str, article_source: str) -> Optional[Dict[str, Any]]:
        ...
        # Simplified direct approach without complex formatting
        system_prompt = """You are a developer writing a dev.to post. Write 500-600 words, first person, opinionated and specific. Be uncomfortable with your take if it feels real.

RESPOND ONLY WITH JSON OBJECT (no markdown, no code blocks):
{{"title": "your title", "body": "your markdown content"}}"""

        user_prompt = f"""Write a dev.to post inspired by: {article_title}

Content: {clean_source_content[:800]}

RESPOND ONLY WITH:
{{"title": "Title in Title Case (under 10 words)", "body": "500-600 word markdown post"}}"""

        try:
            response = self._make_chat_completion_with_fallback(
                payload={
                    "messages": [
                        {"role": "system", "content": system_prompt},
                        {"role": "user", "content": user_prompt},
            ...
            if response is None:
                logger.error("[DEVTO_ARTICLE] All fallback attempts failed")
                return None
            raw = self._extract_message_content(response)
            if raw:
                logger.debug(f"[DEVTO_ARTICLE] Raw ({len(raw)} chars): {raw[:150]}")
            else:
                logger.warning("[DEVTO_ARTICLE] No message content in response")
...
{{
  ...
  "body": "Rewritten body with ## section headers and well-structured paragraphs"
    ...
    def _generate_medium_title_variants(self, base_title: str, body: str, topic: str) -> str:
        ...
        has_learning = re.search(r"\b(?:learned|discovered|realized|found out)\b", body, re.IGNORECASE)
        has_comparison = re.search(r"\b(?:vs|versus|better|faster|slower|cheaper)\b", body, re.IGNORECASE)
        has_warning = re.search(r"\b(?:don't|shouldn't|avoid|watch out|beware)\b", body, re.IGNORECASE)
        
        # Medium-style title templates for variety
        templates = []
        ...
        if has_learning or has_mistake:
            templates.extend([
                f"What I learned about {topic}: {t}",
                f"The real truth about {topic}",
                f"Why I was wrong about {t}",
            ])
        
        # Contrarian/opinion titles
        if has_comparison or has_warning:
            templates.extend([
                f"{t} (and why it matters)",
                f"You're probably doing {topic} wrong",
                f"Stop {topic}: Here's what works instead",
            ])
        
        # Practical/how-to titles
        if re.search(r"\bhow|build|create|deploy\b", body, re.IGNORECASE):
            templates.extend([
                f"How I {t}",
                f"Building {t} the right way",
                f"Practical guide to {t}",
            ])
...
Example structure in body: [Problem explanation] → [Why it matters] → [Solutions including MegaLLM approach] → [Key takeaway]"""
        ...
        try:
            ...
            logger.info(f"[API_REQUEST] URL: dynamic (fallback-enabled)")
            ...
            # Make the API call with fallback logic
            response = self._make_chat_completion_with_fallback(request_payload, timeout=60)
        ...
        try:
            ...
            if response.status_code in [503, 429]:
                if attempt < self.max_retries:
                    ...
                    return self._make_api_call_with_retry(url, payload, attempt + 1, headers=headers, timeout=timeout)
                else:
...
Write a genuine, conversational answer. Return ONLY valid JSON with "title" (exact question) and "body" (your answer)."""
        ...
        try:
            ...
            if response is None:
                logger.error("[QUORA_ANSWER] All fallback attempts failed")
                return None
            raw = self._extract_message_content(response)
            parsed = self._extract_json_object(raw)
            if not parsed:
    ...
    def generate_agiorbust_blog(
        self, topic: str, topic_description: str, keywords: list
    ) -> Optional[Dict[str, str]]:
        """
        Generate a blog post for AGIorBust account focusing on AI automation,
        platform updates, integrations, and industry news (style of manus.im/blog).

        Returns:
            {"title": str, "body": str} or None on error
        """
        logger.info(f"[AGIORBUST_GEN] Starting for topic: {topic}")
        logger.info(f"[AGIORBUST_GEN] Using model: {self.model}")

        system_prompt = f"""You are a technical content writer for AGIorBust, focusing on AI automation,
platform updates, integrations, and third-party connector news. Follow the style of manus.im/blog:

Content focus:
- Latest AI platform releases and feature announcements
- Integration tutorials and third-party connector updates
- Curated AI automation tool reviews and comparisons
- Industry news, company announcements, and partnership stories
- Application showcases and real-world automation workflows

Writing style:
- Instructional and direct — explain how things work and why they matter
- Professional yet innovation-driven — focus on emerging tech and cutting-edge solutions
- Property descriptions use reverse-chronological ordering (newest first)
- Each article includes category tags for easy discovery
- Clear, concise explanations with practical examples
- No marketing fluff or promotional language
- Focus on "reducing X" or "A practical approach to Y" patterns

Content tone characteristics:
- Objective and informative — present facts and insights
- Avoid alarmist language or "killing" narratives
- Use neutral, descriptive language
- Provide actionable insights and practical takeaways
- Include specific details about features, integrations, and use cases
- Title should be specific and descriptive (max 12 words)

Requirements:
- Write original content that offers fresh perspective
- Titles should be descriptive and non-promotional
- Body focuses on practical value, not marketing claims
- Include specific details, metrics, or real-world examples
- Keep language professional but engaging
- Body should include MegaLLM examples where relevant to automation workflows
- Use clear, direct language without unnecessary complexity

Return ONLY valid JSON (no markdown):
{{
  "title": "Your descriptive title (can mention platforms/tools if relevant)",
  "body": "Your blog post content..."
}}"""

        user_prompt = f"""Create a CREATIVE, ORIGINAL blog post for AGIorBust about AI automation,
...
BLOG REQUIREMENTS:
1. Write in an instructional, direct style (how-to, overview, or review format)
2. Focus on AI automation, platforms, integrations, or use cases
3. Include specific details about features, connectors, or applications
4. Provide practical insights and real-world examples
5. Keep language concise, professional, and innovation-focused
6. Avoid marketing/promotional language — stay objective
7. Titles should be descriptive and specific
8. If mentioning MegaLLM, focus on how it enables automation
9. Include bullet points for key takeaways or implementation steps
10. Use terms like "platform," "integration," "connector," "workflow" appropriately

Title guidelines:
- Be descriptive and specific (max 12 words)
- Example: "How X Reduces Y in Z" or "A Practical Guide to [Automation]"
- If reviewing, be clear about what you're reviewing
- Do not use alarmist language like "killing" or "shocking"
- Neutral, professional tone

Body guidelines:
- Break into clear sections if needed
- Use bullet points for implementation steps or key features
- Focus on practical information and user value
- Avoid excessive promotion or endorsements
- Include actionability and real-world application
- Keep it readable for developers and automation engineers

Return valid JSON with title and body only."""

        try:
            logger.info(f"[AGIORBUST_API] URL: dynamic (fallback-enabled)")
            logger.info(f"[AGIORBUST_API] Model: {self.model}")
            logger.info(f"[AGIORBUST_API] Temperature: 0.7, Max Tokens: 2800")
            ...
            if start == -1 or end == -1:
                logger.warning(f"[AGIORBUST_GEN] Invalid JSON object")
                return None
            ...
            if not title or not body:
                logger.warning(f"[AGIORBUST_GEN] Missing title/body content")
                return None

            title = self._sanitize_professional_title(title, topic=topic)
            title, body = self._enforce_megallm_requirements(title, body)
            tags = self._extract_relevant_tags(keywords=keywords, topic=title)
            description = self._build_description_with_tags(body, tags)

            logger.info(f"[AGIORBUST_GEN] Successfully generated: {title[:50]}...")
            return {"title": title, "body": body, "tags": tags, "description": description}
...
{{
  ...
  "body": "Your insightful analysis here..."
        ...
        try:
            ...
            if response is None:
                logger.error("[GEN_ARTICLE] All fallback attempts failed")
                return None
    ...
    def _enforce_megallm_requirements(self, title: str, body: str) -> tuple[str, str]:
        ...
        normalized_title = self._remove_em_dashes((title or "").strip())
        normalized_body = self._remove_em_dashes((body or "").strip())
        ...
        normalized_body = self._remove_em_dashes(normalized_body)
...
{{
  ...
  \"body\": \"Expanded detailed body\"
        ...
        response = self._make_chat_completion_with_fallback(expand_payload, timeout=60)
        if response is None:

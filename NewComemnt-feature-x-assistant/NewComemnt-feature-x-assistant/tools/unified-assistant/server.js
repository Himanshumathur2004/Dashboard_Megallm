...
for (const key of Object.keys(PLATFORMS)) {
  ...
  // SSE endpoint for real-time draft streaming
  app.get(`/api/${key}/draft-stream`, async (_req, res) => {
    ...
    try {
      const progressCb = (progress, drafts) => {
        sendEvent("progress", { progress, drafts });
      };
      const d = await p.draft(progressCb);
      sendEvent("complete", { count: d.length });
    } catch (e) {
...
async function doDraft(){
  ...
  var errEl=document.getElementById('loadError');if(errEl)errEl.textContent='';
  try{
    var evtSource=new EventSource(api(platform)+'/draft-stream');
    evtSource.addEventListener('progress',function(e){
      var data=JSON.parse(e.data);
      if(data.progress&&data.progress.status==='completed'){
        b.textContent='Generating… ('+data.progress.current+'/'+data.progress.total+')';
      }else if(data.progress&&data.progress.status==='processing'){
        b.textContent='Processing @'+(data.progress.authorUsername||'')+' ('+data.progress.current+'/'+data.progress.total+')';
      }
      // Reload drafts from file each time new drafts arrive
    ...
    evtSource.addEventListener('error',function(e){
      if(e.data){
        var data=JSON.parse(e.data);
        errEl.textContent=data.error||'Draft failed';
      }
      evtSource.close();
...
.posted-section{background:#141414;border:1px solid #262626;border-radius:12px;padding:20px;margin-top:20px}
.posted-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px}
.posted-header h2{font-size:16px;color:#fff}
.posted-filters{display:flex;gap:10px;align-items:center}
.posted-filters select{background:#1a1a1a;border:1px solid #333;color:#e8e8e8;padding:6px 12px;border-radius:6px;font-size:13px}
.posted-filters input{background:#1a1a1a;border:1px solid #333;color:#e8e8e8;padding:6px 12px;border-radius:6px;font-size:13px;width:200px}
.posted-filters input::placeholder{color:#666}
.posted-table{display:flex;flex-direction:column;gap:8px;max-height:500px;overflow-y:auto}
.posted-row{background:#1a1a1a;border:1px solid #262626;border-radius:8px;padding:14px;transition:border-color .2s}
.posted-row:hover{border-color:#444}
.posted-row-top{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:10px}
.posted-row-info{flex:1;min-width:0}
.posted-platform{display:inline-block;padding:3px 8px;border-radius:4px;font-size:11px;font-weight:600;margin-bottom:6px}
.posted-title{font-size:14px;color:#fff;margin:0 0 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.posted-title a{color:#fff;text-decoration:none}
.posted-title a:hover{text-decoration:underline}
.posted-meta{font-size:12px;color:#666}
.posted-comment{font-size:13px;color:#aaa;background:#0d0d0d;padding:10px;border-radius:6px;margin-top:8px;line-height:1.5;max-height:80px;overflow:hidden;position:relative}
.posted-comment.collapsed{max-height:40px}
.posted-comment::after{content:'';position:absolute;bottom:0;left:0;right:0;height:20px;background:linear-gradient(transparent,#0d0d0d)}
.posted-score{background:#1a1a1a;border:1px solid #333;padding:6px 12px;border-radius:6px;text-align:center;min-width:50px}
.posted-score-value{font-size:16px;font-weight:700;color:#4ade80}
.posted-score-label{font-size:10px;color:#666}
.posted-empty{text-align:center;color:#666;padding:40px;font-size:14px}
.posted-loading{text-align:center;color:#666;padding:40px;font-size:14px}
</style>
</head>
<body>
<div class="c">
...
async function loadPosted(){
  try{
    const res=await fetch('/api/analytics/posted');
    if(!res.ok)throw new Error('HTTP '+res.status);
    postedData=await res.json();
    renderPosted(postedData);
  }catch(e){
    document.getElementById('postedTable').innerHTML='<div class="posted-empty">Failed to load posted comments</div>';
  }
}

function renderPosted(items){
  const el=document.getElementById('postedTable');
  if(!items.length){
    el.innerHTML='<div class="posted-empty">No posted comments yet</div>';
    return;
  }
  el.innerHTML=items.map(item=>{
    const color=COLORS[item.platform]||'#666';
    const date=item.postedAt?new Date(item.postedAt).toLocaleDateString():'Unknown';
    const time=item.postedAt?new Date(item.postedAt).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):'';
    const commentText=item.comment||'No comment text';
    const shortComment=commentText.length>200?commentText.slice(0,200)+'...':commentText;
    const title=item.postTitle||'Untitled';
    const shortTitle=title.length>80?title.slice(0,80)+'...':title;
    return '<div class="posted-row">'+
      '<div class="posted-row-top">'+
        '<div class="posted-row-info">'+
          '<span class="posted-platform" style="background:'+color+'20;color:'+color+';border:1px solid '+color+'40">'+LABELS[item.platform]+'</span>'+
          '<h3 class="posted-title"><a href="'+item.postUrl+'" target="_blank" rel="noopener">'+escapeHtml(shortTitle)+'</a></h3>'+
          '<div class="posted-meta">'+date+' · '+time+(item.primaryTag?' · '+escapeHtml(item.primaryTag):'')+'</div>'+
        '</div>'+
        '<div class="posted-score">'+
          '<div class="posted-score-value">'+(item.megallmScore||0)+'</div>'+
          '<div class="posted-score-label">SCORE</div>'+
        '</div>'+
      '</div>'+
      '<div class="posted-comment collapsed">'+escapeHtml(shortComment)+'</div>'+
    '</div>';
  }).join('');
}
...
function filterPosted(){
  ...
  if(search){
    filtered=filtered.filter(item=>
      ...
      (item.primaryTag||'').toLowerCase().includes(search)
    );
  }
  renderPosted(filtered);
}

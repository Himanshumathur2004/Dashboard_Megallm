...
raw text only. no "option 1:" labels.`;
  ...
  const raw = await callLLMWithFallback(CONFIG, SYSTEM_PROMPT, prompt, {
    maxTokens: 700,
    temperature: 0.88,
  });
...
export async function draftComments(onProgress = null) {
  ...
  for (let idx = 0; idx < selected.length; idx++) {
    ...
    try {
      ...
      // Save immediately after drafting this post (streaming save)
      await appendDraftsToFile(postDrafts);
      allDrafts.push(...postDrafts);

      // Report progress: finished this post
      if (onProgress) {
        onProgress({
          current: idx + 1,
          total: selected.length,
      ...
      console.log(
        `  ✓ [@${post.by}] ${post.ups}♥: "${post.title.slice(0, 44)}..." [${viral.level.toUpperCase()}] best score ${topScore} (saved ${postDrafts.length})`
      );
    } catch (err) {
  ...
  console.log(
    `\nsaved all options · total drafts: ${allDrafts.length}\n`
  );
  return allDrafts;
}
